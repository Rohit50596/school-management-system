...
def schoolinfo(request):
    try:
        userid=request.session['userid']
        if(request.method=="POST" and request.FILES.get('ppicture')):
            ...
            profilepicture=request.FILES['ppicture']
            signaturepicture=request.FILES['spicture']
...
def balancecheck(request):
    try:
        userid=request.session['userid']
        if(request.method=="POST"):
            studentid=request.POST['search']
            if(Student_sinfo.objects.filter(Student_id=studentid).count()!=0):
                try:
                    form=Student_sinfo.objects.filter(Student_id=studentid)
    ...
    try:
        ...
        if(request.method=="POST"):
            ...
            try:
                ...
                pform=Student_pinfo.objects.get(Student_name=studentname)
                try:
                    if(len(Student_sinfo.objects.filter(Student_name=studentname))==1):
                ...
                messages.error(request,"Student id doesn't match")
                return redirect('/clearfee/')
        else:
    ...
    try:
        ...
        if(request.method=="POST"):
            ...
            try:
                ...
                pform=Student_pinfo.objects.filter(Student_name=searchname)
                try:
                    sform.delete()
                    try:
                        pform.delete()
...
def student(request):
    try:
        ...
        if(request.method=="POST"):
            ...
            searchname=request.POST['searchname']
            try:
                form=Student_sinfo.objects.filter(Student_id__iexact=searchid)
                pform=Student_pinfo.objects.filter(Student_name__iexact=searchname)
...
def us(request):
    try:
        ...
        if(request.method=="POST"):
            ...
            studentname=request.POST['studentname']
            totalfee=request.POST['totalfee']
            form=Student_sinfo.objects.get(Student_id__iexact=studentid)
            pform=Student_pinfo.objects.get(Student_name=studentname)
            form.Student_id=studentid
            form.Student_name=pform
            form.TotalFee=totalfee
